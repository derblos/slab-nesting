...
with st.sidebar:
    ...
    canvas_w = st.number_input("Canvas width (px)", min_value=600, max_value=2400, value=1400, step=50, key="sb_canvas_w")
    canvas_h = st.number_input("Canvas height (px)", min_value=400, max_value=2000, value=950, step=50, key="sb_canvas_h")
...
with st.expander("Add the current shape to the Parts list", expanded=True):
    ...
    if st.button("➕ Add this shape", type="secondary", key="exp_add_btn"):
        if tool == "Rectangle":
                ...
                if w <= 0 or h <= 0:
                    ...
                    if add_cut and cut_list:
                        ...
                        st.session_state.parts.append(Part(
                            id=str(uuid.uuid4()), label=label or f"Rect-{len(st.session_state.parts)+1}",
            ...
            st.warning("Switch to the ‘L-shape (parametric)’ tool to create an L. (The freehand canvas is for rects/polygons.)")
...
if tool == "L-shape (parametric)":
    ...
    with colA:
        ...
        D = st.number_input(f"Depth / thickness D ({_pretty_units(units)})", min_value=0.1, value=25.0, step=0.1, format="%.2f", key="L_D")
    with colB:
        B = st.number_input(f"Outer leg B length ({_pretty_units(units)})", min_value=1.0, value=60.0, step=0.5, format="%.2f", key="L_B")
    ang = st.number_input("Inside angle (degrees)", min_value=30.0, max_value=150.0, value=90.0, step=1.0, format="%.0f", key="L_angle")
    ...
    with st.expander("Add this L-shape", expanded=True):
        qtyL = st.number_input("Quantity", min_value=1, step=1, value=1, key="L_qty")
        labelL = st.text_input("Label (optional)", value="", key="L_label")
        allow_rotL = st.checkbox("Allow rotation for this part (0/90°)", value=True, key="L_rot")

        add_cutL = st.checkbox("Add cutout(s) to this L-shape", key="L_cut_toggle")
        cut_listL: List[Tuple[float,float,float,float]] = []
        if add_cutL:
            ncuts = st.number_input("Number of cutouts", min_value=1, max_value=5, value=1, step=1, key="L_cut_count")
            for i in range(ncuts):
                st.write(f"Cutout #{i+1}")
                c1, c2, c3, c4 = st.columns(4)
                with c1:
                    cw = st.number_input(f"W{i+1} ({_pretty_units(units)})", min_value=0.1, value=30.0, step=0.1, format="%.2f", key=f"L_cw_{i}")
                with c2:
                    ch = st.number_input(f"H{i+1} ({_pretty_units(units)})", min_value=0.1, value=20.0, step=0.1, format="%.2f", key=f"L_ch_{i}")
                with c3:
                    cx = st.number_input(f"X{i+1} offset", value=10.0, step=0.5, format="%.2f", key=f"L_cx_{i}")
                with c4:
                    cy = st.number_input(f"Y{i+1} offset", value=10.0, step=0.5, format="%.2f", key=f"L_cy_{i}")
                cut_listL.append((cx, cy, cw, ch))

        if st.button("➕ Add L-shape", type="secondary", key="btn_add_L"):
            meta = {"is_L": True, "A": float(A), "B": float(B), "tA": float(D), "tB": float(D), "angle": float(ang)}
            ...
            st.session_state.parts.append(Part(
                id=str(uuid.uuid4()), label=labelL or f"L-{len(st.session_state.parts)+1}",
                qty=int(qtyL), shape_type="polygon",
                width=None, height=None, points=pts_out,
                allow_rotation=bool(allow_rotL),
                meta=meta
            ))
            st.session_state.needs_nest = True
            st.success("Added L-shape" + (" (with cutout(s))." if (add_cutL and cut_listL) else "."))
