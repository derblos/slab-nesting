...
with st.sidebar:
    ...
    canvas_w = st.number_input("Canvas width (px)", min_value=600, max_value=2000, value=1200, step=50, key="sb_canvas_w")
    canvas_h = st.number_input("Canvas height (px)", min_value=400, max_value=1400, value=700, step=50, key="sb_canvas_h")
...
with left:
    ...
    if tool == "Rectangle":
        use_live_drawer = st.toggle("Use live drawer (beta)", value=True, help="Shows W×H while dragging (Konva). Turn off to use classic canvas with bulk-add.", key="toggle_live_drawer")
    ...
    if tool == "Rectangle" and use_live_drawer:
        ...
        if add_live_clicked:
            # This injects a tiny form that grabs window.getLiveRect() from the iframe and writes it to the parent URL
            components.html("""
    ...
    if raw:
        try:
            ...
            if w <= 0 or h <= 0:
                ...
                if use_live_drawer and add_cut_live and cut_list_live:
    ...
    # Classic canvas flow (polygons + bulk add)
